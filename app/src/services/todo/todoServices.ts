   ...
   getTodoById = async (req:any) => {
    try {
     ...
     const todo = await Todo.findOne({
      ...
      } else return {statusCode : statusCode.SUCCESS,message : todoEntity.createFromObject(todo)};
    } catch (err) {
      console.log(err);
      return {statusCode : statusCode.SERVER_ERROR , message : message.SERVER_ERROR}  
    }
  };
   addTodo = async (req:any) => {
    try {  
      const body = req.body;
